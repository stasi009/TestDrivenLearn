...
class Sampling(object):
    ...
    def __init__(self, params):
        self.params = params
        self.prep = BatchGenerator(None, 2, self.params.batch_size)

        self.sequence = tf.placeholder(tf.float32, [1, 2, len(self.prep.VOCABULARY)])
        self.state = tf.placeholder(tf.float32,[self.params.rnn_layers, 1, self.params.rnn_hidden])
        self.model = PredictiveCodingModel(self.params, self.sequence, self.state)
    ...
    def __call__(self, seed, length=100):
        text = seed
        state = [np.zeros((1, self.params.rnn_hidden)) for _ in xrange(self.params.rnn_layers)]

        for _ in range(length):
            feed = {
                self.state: np.stack(state),
                self.sequence: self.prep([text[-1] + '?'])
            }
            prediction, state = self.sess.run([self.model.prediction, self.model.state], feed)
