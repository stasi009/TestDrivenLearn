...
class ListComprehensionTest(unittest.TestCase):
    ...
    def testAsFilter(self):
        odd_list = [x for x in range(6) if x%2==1]
        self.assertEqual([1,3,5],odd_list)
        
        even_list = [x for x in range(6) if x%2==0]
        self.assertEqual([0,2,4],even_list)
        
    def testAsMap(self):
        self.assertEqual([0,1,4,9],[x*x for x in range(4)])
        self.assertEqual([1,2,3,4],[x+1 for x in range(4)])
        
    def testMultiIterVariables(self):
        self.assertEqual([(0,0),(0,1),
                          (1,0),(1,1),
                          (2,0),(2,1)],[(x,y) for x in range(3) for y in range(2)])
