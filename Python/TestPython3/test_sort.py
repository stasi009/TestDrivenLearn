...
class TestSort(unittest.TestCase):
    ...
    def setUp(self):
        self._stud1 = TestSort.Student(1,"cheka")
        self._stud2 = TestSort.Student(2,"stasi")
        self._stud3 = TestSort.Student(3,"mss")
    ...
    def testSorted_ByKey(self):
        sorted_by_no = sorted(self._unsorted,key=TestSort.Student.getNumber)
        self.assertEqual(self._number_sorted,sorted_by_no)
        
        sorted_by_name = sorted(self._unsorted,key=TestSort.Student.getName)
    ...
    def testInplaceSort(self):
        alist = [person.Person(x,"p%d"%x) for x in range(4,0,-1)]
        ascending_list = [person.Person(x,"p%d"%x) for x in range(1,5)] 
