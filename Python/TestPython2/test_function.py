...
class FunctionTest(unittest.TestCase):
    ...
    def testVariableListArgs(self):
        ...
        # convert a sequence into optional arguments
        s = "xy"
        self.assertEqual((1,"xy"),_fool(1,s))
        self.assertEqual((1,"x","y"),_fool(1,*s))
    ...
    def testPassMutableArgs1(self):
        """
        this testcase checks that parameter passing is always "address passing" in python
        """
        def simple_function(alist,total):
            for x in xrange(total):
                alist.append(x)
        
        empty_list = []
        simple_function(empty_list,3)
        
        self.assertEqual([0,1,2],empty_list)
        
    def testPassMutableArgs2(self):
        """
        this testcase checks that parameter passing is always "address passing" in python
        """
        def simple_function(alist,total):
            alist = range(total) # redirect to a totally new object
            return alist
        
        empty_list = []
        returned_list = simple_function(empty_list,3)
        
        self.assertEqual([],empty_list)
        self.assertEqual([0,1,2],returned_list)
        
    def testPassImmutableArgs(self):
        def _fool(num):
            # No outside effect; lets the local int to point to a new int object,
    ...
    def test_delegate2(self):
        ...
        # pass in a instance method as delegate
        colors = collections.defaultdict(missing_counter.default,{"green": 12, "blue": 3})

        for k,v in [("red",5),("blue",17),("orange",9)]:
            colors[k] += v

        # check the result
        self.assertEqual(2,missing_counter.num_missing)
        self.assertEqual({"green": 12, "blue": 20,"red":5,"orange":9},dict(colors))
